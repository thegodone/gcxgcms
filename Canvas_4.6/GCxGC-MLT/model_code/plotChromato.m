function [varargout] = plotChromato(Chromato,varargin) % alternative use: plotChromato(X,Y,Chromato,varargin)
...
% - FirstDshift: this is the shift for the first dimension first pixel
% (i.e. time is not starting at 0, but at FirstDshift). Give it as seconds
...
% 
% File written by J.G., last modified on 9th July 2014.
% 
% 12th June 2014:
% Added 'FirstDshift'
% 9th July 2014:
% Displaced the definition of [nb_measY,nb_measX] after the operations done
% with input as a structure.
% 20th November 2014:
% Added the possibility to do:
% Figure_handle = plotChromato(Chromato)
% Then 'Figure_handle' can be used to refer to the ploted graph in matlab
% commands.
% 
%
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
for i=cnt:2:length(varargin)
    ...
    elseif strcmpi(varargin{i},'FirstDshift')
...
if ~exist('FirstDshift','var')
    FirstDshift = 0; % by default, no shift.
end
...
%         nb_measX=size(Chromato,2); % [pixel]
        if ~exist('X','var')
            ...
            X = FirstDshift+ones(MP*SR,1)*(1:nb_measX)*MP/60; % 60 = 60 [s]
        end
        fig_ref = pcolor(X,Y,Chromato); shading interp
    elseif strcmpi(ShadingMode,'flat')
        nb_measX=size(Chromato,2)+1; % [pixel]
        if ~exist('X','var')
            Y = ((1:(MP*SR+1))'/SR)*ones(1,nb_measX);
            X = FirstDshift+ones(MP*SR+1,1)*(1:nb_measX)*MP/60; % 60 = 60 [s]
        else
            XBis = ones(size(X)+1);
...
%             of a given flat value box, not at the left of it.
        ...
        fig_ref = pcolor(X,Y,Chromato); shading flat
    end
...
%             pcolor(Chromato); shading interp
            ...
            X = FirstDshift + ones(nb_measY,1)*(1:nb_measX); % pixels
            fig_ref = pcolor(X,Y,Chromato); shading interp
        else
            fig_ref = pcolor(X,Y,Chromato); shading interp
        end
    elseif strcmpi(ShadingMode,'flat')
        ...
        if ~exist('X','var')
            ...
            X = FirstDshift+ones(size(Chromato,1),1)*(1:nb_measX); % 60 = 60 [s]
        else
            XBis = ones(size(X)+1);
            XBis(1:end-1,1:end-1) = X;
            XBis(:,end) = XBis(:,end-1) + (XBis(:,end-1)-XBis(:,end-2));
