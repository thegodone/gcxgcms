function [i,j,FOUND] = csmax(cs,j0,NOISE,RISE,LEVEL,RISE2,LEVEL2)
    i0 = j0;
    countdown = 2;
    N = length(cs);
    while (j0 < N)
        if (cs(j0+1,3) < cs(j0,3)) 
            j0 = j0 + 1;
        elseif (i0 == j0)
            j0 = j0+1;
            i0 = j0;
        elseif (cs(i0,3)-cs(j0,3) < NOISE)
            if (cs(j0+1,3) >= cs(i0,3))
                i0 = j0+1;
                j0 = i0;
            else
                j0 = j0+1;
